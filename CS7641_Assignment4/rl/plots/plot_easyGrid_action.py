...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_valueI_action.png')
plt.close()
...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_policyI_action.png')
plt.close()

Q_action = [int(numeric_string) for numeric_string in Q_action ]
plt.plot(range(1,1001), Q_action, c = 'g',linewidth ='1')
plt.xlabel('Iterations',  fontsize = 13)
plt.ylabel('Actions',  fontsize = 13)
# plt.legend(('EM', 'Kmeans'), loc = 4, fontsize = 13)
plt.ylim(0,1000)
...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_Q_action.png')
plt.close()