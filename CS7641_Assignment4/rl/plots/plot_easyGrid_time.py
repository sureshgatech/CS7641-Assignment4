...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_valueI_time.png')
plt.close()
...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_policyI_time.png')
plt.close()

Q_time = [int(numeric_string) for numeric_string in Q_time ]
plt.plot(range(1,1001), Q_time, c = 'g',linewidth ='1')
plt.xlabel('Iterations',  fontsize = 13)
plt.ylabel('Time',  fontsize = 13)
# plt.legend(('EM', 'Kmeans'), loc = 4, fontsize = 13)
plt.ylim(0,1000)
...
plt.grid(True)
#plt.show()
plt.savefig('easy_grid_Q_time.png')
plt.close()